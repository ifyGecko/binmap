...
Requirements: `pkg-config`, a C11 compiler, **SDL2** (`libsdl2-dev` /
`sdl2-devel`), and a Linux system.
...
└── src/
    ...
    └── main.c       SDL2 init, mmap, event loop, status bar, legend
```

## Limitations

- Linux only. (SDL2 makes porting cheap; not yet packaged.)
